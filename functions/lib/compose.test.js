...
function createMockRequest(data, auth = { uid: 'test-user' }) {
    return { data, auth };
}
// Valid test params
const validParams = {
...
describe('compose Cloud Function', () => {
    ...
    describe('Authentication', () => {
        it('throws unauthenticated error when request.auth is null', async () => {
            ...
            await expect(compose_1.compose.run(request)).rejects.toThrow('User must be logged in to use this service.');
        });
        it('proceeds with authenticated requests', async () => {
            const request = createMockRequest(validParams);
            const result = await compose_1.compose.run(request);
    ...
    describe('Input Validation - Required Fields', () => {
        it('throws invalid-argument for missing mode', async () => {
            const params = Object.assign(Object.assign({}, validParams), { mode: undefined });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Missing required parameters.');
        });
        it('throws invalid-argument for missing root', async () => {
            const params = Object.assign(Object.assign({}, validParams), { root: undefined });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Missing required parameters.');
        });
        it('throws invalid-argument for missing tempo', async () => {
            const params = Object.assign(Object.assign({}, validParams), { tempo: undefined });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Missing required parameters.');
        });
        it('throws invalid-argument for missing instruments', async () => {
            const params = Object.assign(Object.assign({}, validParams), { instruments: undefined });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Missing required parameters.');
        });
        it('throws invalid-argument for missing mood', async () => {
            const params = Object.assign(Object.assign({}, validParams), { mood: undefined });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Missing required parameters.');
        });
    });
    describe('Input Validation - Invalid Values', () => {
        it('throws invalid-argument for invalid mode value', async () => {
            const params = Object.assign(Object.assign({}, validParams), { mode: 'invalid' });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Invalid mode: invalid. Must be one of: gong, shang, jue, zhi, yu');
        });
        it('throws invalid-argument for invalid mood value', async () => {
            const params = Object.assign(Object.assign({}, validParams), { mood: 'angry' });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Invalid mood: angry. Must be one of: calm, heroic, melancholic, festive');
        });
        it('throws invalid-argument for invalid instrument in array', async () => {
            const params = Object.assign(Object.assign({}, validParams), { instruments: ['erhu', 'guitar'] });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Invalid instrument: guitar. Must be one of: erhu, guzheng, pipa, dizi');
        });
        it('throws invalid-argument for tempo below 40', async () => {
            const params = Object.assign(Object.assign({}, validParams), { tempo: 20 });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Tempo must be a number between 40 and 160 BPM.');
        });
        it('throws invalid-argument for tempo above 160', async () => {
            const params = Object.assign(Object.assign({}, validParams), { tempo: 200 });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Tempo must be a number between 40 and 160 BPM.');
        });
        it('throws invalid-argument for empty instruments array', async () => {
            const params = Object.assign(Object.assign({}, validParams), { instruments: [] });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('At least one instrument must be selected.');
        });
        it('throws invalid-argument for non-array instruments', async () => {
            const params = Object.assign(Object.assign({}, validParams), { instruments: 'erhu' });
            const request = createMockRequest(params);
            await expect(compose_1.compose.run(request)).rejects.toThrow('At least one instrument must be selected.');
        });
    });
    describe('AI Integration', () => {
        it('calls Gemini with prompt including mode, root, tempo, instruments, mood', async () => {
            const request = createMockRequest(validParams);
            await compose_1.compose.run(request);
        ...
        it('returns sanitized internal error on AI failure (no stack traces)', async () => {
            ...
            const request = createMockRequest(validParams);
            await expect(compose_1.compose.run(request)).rejects.toThrow('Unable to generate composition at this time. Please try again.');
        });
    });
    describe('Valid Modes', () => {
        ...
        validModes.forEach((mode) => {
            it(`accepts valid mode: ${mode}`, async () => {
                const params = Object.assign(Object.assign({}, validParams), { mode });
                const request = createMockRequest(params);
                const result = await compose_1.compose.run(request);
                expect(result).toBeDefined();
            });
    ...
    describe('Valid Moods', () => {
        ...
        validMoods.forEach((mood) => {
            it(`accepts valid mood: ${mood}`, async () => {
                const params = Object.assign(Object.assign({}, validParams), { mood });
                const request = createMockRequest(params);
                const result = await compose_1.compose.run(request);
                expect(result).toBeDefined();
            });
